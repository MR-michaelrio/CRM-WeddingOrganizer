...
export type BuildInvoiceOpts = {
  /** Saat true: hilangkan toolbar & auto-print script, inject <base> agar aset
   *  relatif (logo/signature) ter-resolve — dipakai untuk render PDF. */
  forPdf?: boolean;
  /** Origin app untuk <base href>, mis. "http://localhost:3005". */
  baseUrl?: string;
};

export type BuiltInvoice = { html: string; number: string; clientName: string };

/** Bangun HTML invoice yang sama persis dengan route print. */
export async function buildInvoiceHtml(
  ...
  const baseTag = opts.baseUrl ? `<base href="${escapeHtml(opts.baseUrl)}/">` : "";
  const actionsHtml = opts.forPdf
    ? ""
    : `<div class="actions">
    <button onclick="window.print()">Print</button>
    <button class="secondary" onclick="window.close()">Close</button>
  </div>`;
  const autoPrintScript = opts.forPdf
    ? ""
    : `<script>
...
<body>
  ...
  return { html, number: invoice.number, clientName: invoice.client.names };
}
