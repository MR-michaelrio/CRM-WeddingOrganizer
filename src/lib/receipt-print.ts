...
export type BuildReceiptOpts = {
  forPdf?: boolean;
  baseUrl?: string;
};

export type BuiltReceipt = { html: string; number: string; clientName: string };

/** Bangun HTML kwitansi yang sama persis dengan route receipt. */
export async function buildReceiptHtml(
  id: number,
  opts: BuildReceiptOpts = {}
): Promise<BuiltReceipt | null> {
